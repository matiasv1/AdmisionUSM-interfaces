<template>
   <div class="pa-0">

  
  <NavbarSinLogin v-if="tipo ==1"></NavbarSinLogin>
  <NavbarConLogin v-else-if="tipo == 2"></NavbarConLogin>

   
   <v-container-fluid
    class="mt-15"
    fluid
  >
      <v-row>
        <v-col>
            <v-card style="margin-top:70px" text :to="{path:'/sesion-iniciada'}">
            <v-img
              src="data:image/png;base64,"
              height=auto
              max-width="300"
              class="grey darken-4"
              
            ></v-img>

            </v-card>
        </v-col>
      
      </v-row>
      
          <v-card>
            <v-img
              src="https://admision.usm.cl/banner-inicio.jpg"
              height="300"
              max-width="auto"
              class="grey darken-4"
            >
            <v-card-title class="white--text text-h4" style="margin-top: 200px; margin-left:190px;">Bienvenido a las Actividades</v-card-title>
          </v-img>
            
          </v-card>
   
    
    
    <div class="layout">
      <v-container fluid class="px-2 mt-1">
        <v-breadcrumbs :items="items" class="text-h6">
          <template v-slot:divider>
            <v-icon>mdi-chevron-right</v-icon>
        </template>
        </v-breadcrumbs>

        <v-row>
            <v-col v-for="card in cards" :key="card.title" :cols="card.flex" class="px-3 py-3 ">
              <v-card max-width="700px" class="Opacity txt">
                <v-img :src="card.src" class="white--text align-end"
                  gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)" height="350px" max-width="500px">
                  <v-card-title v-text="card.title" class="justify-center txt" style="margin-bottom:0%" ></v-card-title>
                  <v-card-text v-text="card.text2"></v-card-text>
                  <v-card-text v-text="card.text"></v-card-text>
                </v-img>

                
              </v-card>
            </v-col>
          </v-row>

  
      </v-container>
    </div>
    
    </v-container-fluid>





  </div>

  
  
</template>
<script>
import NavbarSinLogin from '../components/NavbarSinLogin.vue';
import NavbarConLogin from '../components/NavbarConLogin.vue';


export default {
  name: 'actividades',
  props:['tipo'],
  components: {NavbarSinLogin, NavbarConLogin},

  data: () => ({
    cards: [
      { title: 'Charla informativa Programa Piloto Comercial', src: 'https://admision.usm.cl/beneficio.jpg', flex: 3, text:'Nov 28, 2022 9:00-13:00 (GMT-04:00))' },
      { title: '30a Feria de SOFTWARE                 ', src: 'https://scotiabankfiles.azureedge.net/scotiabank-chile/img/landing-page/rse/software-feria.png', flex: 3 , text:'Nov 25, 2022 11:30-17:00 (GMT))'},
      { title: 'Visita Casa Central - Colegio Nuevo Chileno Británico', src: 'https://admision.usm.cl/postula.jpg', flex: 3 , text:'Nov 30, 2022 9:00-13:00 (GMT-04:00))'},
      { title: 'Visita Sede: Colegio San Jose Cabrero, taller informática', src: 'https://admision.usm.cl/homehero.jpg', flex: 3, text:'Dic 1 27, 2022 10:00-13:00 (GMT)' },

    ],
    items: [
        {
          text: 'Inicio',
          disabled: false,
          href: '/',
        },
        {
          text: 'Informaciones',
          disabled: false,
          href: '/informacion/2',
        },
        {
          text: 'Actividades',
          disabled: true,
          href: '/about',
        },
      ]
    
  }),
  
};
</script>   
<style>
.layout {
  max-width: 75%;
  text-align: center;
  margin: 0 auto
}

.h1_layout {
  margin-top: 20px;
  max-width: 75%;
  margin: 0 auto;
  padding-left: 5%;
}
.txt:hover {
    text-decoration: underline;
    text-decoration-color: white;
}
</style>