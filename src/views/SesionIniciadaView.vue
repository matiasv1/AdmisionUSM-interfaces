<template>
  <div class="pa-0">

    <NavbarConLogin></NavbarConLogin>
    

    <v-container-fluid class="mt-15" fluid>

      <v-row>
        <v-col>
          <v-card style="margin-top:90px" text :to="{ path: '/sesion-iniciada' }">
            <v-img
              src="data:image/png;base64,"
              height=auto max-width="300" class="grey darken-4" style="margin:10px 10px 0 100px"></v-img>

          </v-card>
        </v-col>

      </v-row>

      <v-card style="padding-top:15px ;">
        <v-img src="https://admision.usm.cl/banner-inicio.jpg" height="300" max-width="auto" class="grey darken-4">
          <v-card-title class="white--text text-h4" style="margin-top: 200px; margin-left:190px;">Bienvenido a la UTFSM
          </v-card-title>
        </v-img>

      </v-card>


    </v-container-fluid>

    <v-container class="">
      <v-row class="">
        <v-col class="col-8">
          <h1>ADMISIÓN UTFSM</h1>
        </v-col>
        <v-col class="col-4">
          <h1>Conoce nuestra oferta</h1>
        </v-col>
      </v-row>
      <v-row>
        <v-col class="col-8 pr-5">
          <v-hover text>
            <template v-slot:default="{ hover }">
              <div class="Opacity">
                <div :class="`elevation-${hover ? 24 : 6}`" class="mx-auto pa-0 transition-swing ">
                  <v-card :to="{ path: '/postulaciones' }">
                    <v-img max-height="240px" src="https://admision.usm.cl/postula.jpg"
                      gradient="to top right, rgba(0,0,0,0.6), rgba(0,0,0,0)">
                      <v-card-title class="text-h4 white--text pb-0" style="margin-top: 110px; margin-left:50px;">
                        Postula
                        Aquí</v-card-title>
                      <v-card-text class=" white--text ml-13 "> Postula online a nuestras carreras y programas
                      </v-card-text>

                    </v-img>
                  </v-card>
                </div>
              </div>
            </template>
          </v-hover>

        </v-col>
        <v-col class="col-4">
          <v-simple-table class="elevation-2 rounded">
            <tbody>
              <tr>
                <td>
                  <a href="https://usm.cl/admision/carreras/" style="text-decoration:none">
                    <h4 class="blue--text text--darken-4"> Carreras Pregado Diurno</h4>
                  </a>
                </td>
              </tr>
              <tr>
                <td>
                  <a href="https://usm.cl/admision/carreras/" style="text-decoration:none">
                    <h4 class="blue--text text--darken-4"> Carreras Pregado Vespertino</h4>
                  </a>
                </td>
              </tr>
              <tr>
                <td>
                  <a href="https://usm.cl/admision/carreras/" style="text-decoration:none">
                    <h4 class="blue--text text--darken-4"> Magísteres</h4>
                  </a>
                </td>
              </tr>
              <tr>
                <td>
                  <h4 class="blue--text text--darken-4"> Doctorados</h4>
                </td>
              </tr>
              <tr>
                <td>
                  <h4 class="blue--text text--darken-4"> Educación continua</h4>
                </td>
              </tr>
            </tbody>
          </v-simple-table>
        </v-col>
      </v-row>
      <v-row class="mt-7">
        <v-col class="col-8">
          <h1>Tour virtual 360°</h1>
        </v-col>
      </v-row>
      <v-row>
        <v-col class="col-10">
          <div class="Opacity">
                <div :class="`elevation-${hover ? 24 : 6}`" class="mx-auto pa-0 transition-swing">
                  <v-card  href="https://tour360.usm.cl/">
                    <v-img max-height="240px" src="https://admision.usm.cl/tour-virtual.jpg" gradient="to top right, rgba(0,0,0,0.6), rgba(0,0,0,0)" class="rounded-2">
                      <v-card-title class="white--text pb-0" style="margin-top: 130px; margin-left:50px;">
                        Ver recorridos virtuales</v-card-title>
                      

                    </v-img>
                  </v-card>
                </div>
              </div>
        </v-col>
      </v-row>

      <v-row class="mt-7">
        <v-col class="col-8 ">
          <h1>Descubre tu futuro USM</h1>
        </v-col>
      </v-row>
      <v-row>
        <v-col class="col-10">
          <div class="Opacity">
                <div :class="`elevation-${hover ? 24 : 6}`" class="mx-auto pa-0 transition-swing">
                  <v-card  href="https://admision.usm.cl/landpro/">
                    <v-img max-height="240px" src="https://admision.usm.cl/novedadesusm.png" gradient="to top right, rgba(0,0,0,0.6), rgba(0,0,0,0)" class="rounded-2">
                      <v-card-title class=" white--text ml-13 " style="margin-top: 150px; margin-left:50px;"> Inscríbete aquí para recibir más información, novedades y actividades de admisión usm
                      </v-card-title>
                      

                    </v-img>
                  </v-card>
                </div>
              </div>
        </v-col>
      </v-row>


      <v-row class="mt-7">
        <v-col class="col-8 ">
          <h1>Programa IDP Crea</h1>
        </v-col>
      </v-row>
      <v-row>
        <v-col class="col-10">
          <div class="Opacity">
                <div :class="`elevation-${hover ? 24 : 6}`" class="mx-auto pa-0 transition-swing">
                  <v-card  href="https://admision.usm.cl/landpro/">
                    <v-img max-height="240px" src="https://admision.usm.cl/programa_idp_crea.png" gradient="to top right, rgba(0,0,0,0.6), rgba(0,0,0,0)" class="rounded-2">
                      <v-card-title class=" font-weight-medium white--text ml-13 " style="margin-top: 150px; margin-left:50px;"> Postula aquí al programa de ingeniería y creatividad aplicada
                      </v-card-title>
                      

                    </v-img>
                  </v-card>
                </div>
              </div>
        </v-col>
      </v-row>

    </v-container>
    <Footer />


  </div>

</template>
<script>
import NavbarConLogin from '../components/NavbarConLogin.vue';
import Footer from '@/components/Footer.vue'
export default {
  name: 'App',
  components: { NavbarConLogin, Footer },

  data: () => ({
    dialogsesion:true,
  }),

};
</script>
<style>
.layout {
  max-width: 75%;
  text-align: center;
  margin: 0 auto
}

.h1_layout {
  margin-top: 20px;
  max-width: 75%;
  margin: 0 auto;
  padding-left: 5%;
}
</style>